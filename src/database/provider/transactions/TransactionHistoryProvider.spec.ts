...
describe('Realiza o pagamento de um boleto', () => {
    beforeAll(async () => {
        ...
        await knex(TableNames.account).insert(transactions);

    });

    // Usuário 1
    test('Consulta a lista de transações ', async () => {
    ...
    test('Consulta a lista de transações ', async () => {
        const transactionHistoryProvider = new TransactionHistoryProvider();
        expect((await transactionHistoryProvider.getAllByUserId(1))?.length).toBe(9);
    });
});
